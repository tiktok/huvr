...
def interpolate_pos_embed(model, checkpoint_model,use_decoder=True):
    if 'transformer_encoder.pos_embed' in checkpoint_model and 'transformer_encoder.pos_embed' in model.state_dict():
        pos_embed_checkpoint = checkpoint_model['transformer_encoder.pos_embed']
        embedding_size = pos_embed_checkpoint.shape[-1]
        num_patches = model.transformer_encoder.patch_embed.num_patches
        num_extra_tokens = model.transformer_encoder.pos_embed.shape[-2] - num_patches
        ...
        if orig_size != new_size:
            ...
            checkpoint_model['transformer_encoder.pos_embed'] = new_pos_embed
        else:
            print("No need to interpolate position embedding",orig_size,new_size)
    if "tokenizer.decoder_posemb" in checkpoint_model and \
        "tokenizer.decoder_posemb" in model.state_dict() and \
        use_decoder ==True:
        pos_embed_checkpoint = checkpoint_model['tokenizer.decoder_posemb']
        embedding_size = pos_embed_checkpoint.shape[-1]
        num_patches = model.transformer_encoder.patch_embed.num_patches
        ...
        if orig_size!=new_size:
            ...
            pos_tokens = torch.nn.functional.interpolate(
                pos_tokens, size=(new_size, new_size), mode='bicubic', align_corners=False).squeeze()
            pos_tokens = pos_tokens.permute(1, 2, 0).flatten(0, 1)
            checkpoint_model['tokenizer.decoder_posemb'] = pos_tokens
